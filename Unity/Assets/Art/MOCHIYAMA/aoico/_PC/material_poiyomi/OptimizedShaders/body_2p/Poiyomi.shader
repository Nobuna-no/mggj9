Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/8f4affb8ccf45874890bafc532f2aa1a"
{
	Properties
	{
...
#define PROP_LIGHTINGADDITIVELIMIT 0.5
#define PROP_LIGHTINGADDITIVEMONOCHROMATIC 0.5
...
#define PROP_SHADOWBORDER 1
#define PROP_SHADOWBLUR 0
#define PROP_SHADOW2NDCOLORTEXUV 0
#define PROP_SHADOW2NDBORDER 0
#define PROP_SHADOW2NDBLUR 0
#define PROP_SHADOW3RDCOLORTEXUV 0
#define PROP_SHADOW3RDBORDER 0
#define PROP_SHADOW3RDBLUR 0
...
#define PROP_BASECOLOR_STEP 0.001
#define PROP_BASESHADE_FEATHER 0.01
#define PROP_SHADECOLOR_STEP 0.181
#define PROP_1ST2ND_SHADES_FEATHER 0.541
...
#define PROP_RIMLIGHT_POWER 0.253
#define PROP_RIMLIGHT_INSIDEMASK 0.098
#define PROP_RIMLIGHT_FEATHEROFF 1
#define PROP_LIGHTDIRECTION_MASKON 1
...
#define PROP_TWEAK_RIMLIGHTMASKLEVEL -0.03
...
#define PROP_TWEAK_HIGHCOLORMASKLEVEL -0.394
...
#define PROP_HIGHCOLOR_POWER 0.532
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			{
				float MainColorFeatherStep = float(0.001) - float(0.01);
				float firstColorFeatherStep = float(0.181) - float(0.541);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				firstShadeMap.rgb *= float4(0.332847,0.4368164,0.9786729,1).rgb; //* lighColor
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.001) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0.181) - firstColorFeatherStep) * (shadowMask));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lns.x = lilTooningNoSaturate(poiLight.lightMap, float(1), float(0));
				lns.y = lilTooningNoSaturate(poiLight.lightMap, float(0), float(0));
				lns.z = lilTooningNoSaturate(poiLight.lightMap, float(0), float(0));
				lns.w = lilTooningNoSaturate(poiLight.lightMap, float(1), float(0), float(0));
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.8176128,0.512557,0.4782603,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.8176128,0.512557,0.4782603,1).rgb;
				}
				if (float4(1,1,1,0).a > 0)
				{
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(1,1,1,0).rgb;
					lns.y = float4(1,1,1,0).a - lns.y * float4(1,1,1,0).a;
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0.7892805,0.3751924,1).rgb);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.9334825,0.8631573,1).rgb, float(0)), float4(1,0.9334825,0.8631573,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.9334825,0.8631573,1).rgb, float(0)), float4(1,0.9334825,0.8631573,1).a);
				#endif
				...
				rim *= float4(1,0.9334825,0.8631573,1).a * rimColor.a * rimMask;
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.9334825,0.8631573,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.253))));
				float _Rimlight_InsideMask_var = saturate(lerp((0.0 + ((_RimLightPower_var - float(0.098)) * (1.0 - 0.0)) / (1.0 - float(0.098))), step(float(0.098), _RimLightPower_var), float(1)));
				float _VertHalfLambert_var = 0.5 * dot(poiMesh.normals[0], poiLight.direction) + 0.5;
				float3 _LightDirection_MaskOn_var = lerp((_Is_LightColor_RimLight_var * _Rimlight_InsideMask_var), (_Is_LightColor_RimLight_var * saturate((_Rimlight_InsideMask_var - ((1.0 - _VertHalfLambert_var) + float(0))))), float(1));
				...
				float3 _RimLight_var = (saturate((_Set_RimLightMask_var.g + float(-0.03))) * lerp(_LightDirection_MaskOn_var, (_LightDirection_MaskOn_var + (lerp(ApRimColor, (ApRimColor * poiLight.directColor), float(1)) * saturate((lerp((0.0 + ((_ApRimLightPower_var - float(0.098)) * (1.0 - 0.0)) / (1.0 - float(0.098))), step(float(0.098), _ApRimLightPower_var), float(0)) - (saturate(_VertHalfLambert_var) + float(0)))))), float(0)));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.attenuation = attenuation;
				#endif
				poiLight.directColor = float(1) ? min(float(0.5), _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
