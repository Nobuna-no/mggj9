Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/f73d080774b2fe6478bf94fdf50a0be5"
{
	Properties
	{
...
#define PROP_MATCAPUVMODE 2
#define PROP_MATCAPCOLORTHEMEINDEX 0
#define PROP_MATCAP
#define PROP_MATCAPBORDER 0.288
...
#define PROP_RIMLIGHT_POWER 0.031
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			{
				float MainColorFeatherStep = float(1) - float(0.0001);
				float firstColorFeatherStep = float(0) - float(0.541);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float4(0.3940328,0.5316101,0.8960326,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.3940328,0.5316101,0.8960326,1).rgb;
				}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(2))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.288) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.288) + 0.5;
						break;
					}
					case 2:
					{
				...
				blendMatcap(poiLight, poiFragData, float(0.3), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.1350997,0.2495636,0.280335,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.031))));
